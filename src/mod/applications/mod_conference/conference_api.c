...
api_command_t conference_api_sub_commands[] = {
        ...
        {"list-member-vid-layout", (void_fn_t) & conference_api_sub_list_member_vid_layout, CONF_API_SUB_ARGS_SPLIT, "list-member-vid-layout", "<member_id>"}
...
switch_status_t conference_api_sub_list_member_vid_layout(conference_obj_t *conference, switch_stream_handle_t *stream, int argc, char **argv)
{
        ...
        member = conference_member_get(conference, i);
        if(member){
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, " Member ID : %d Canvas ID : %d , VideoLayerID : %d\n", i,member->canvas_id,member->video_layer_id);

                                if (conference->canvases[member->canvas_id]) {
                                        conference_api_layout(conference, conference->canvases[member->canvas_id], conference->canvases[member->canvas_id]->vlayout,stream);
                                } else {
                                        stream->write_function(stream, "-ERR  Canvas ID: %d does not exist", member->canvas_id);
                                }
        ...
        switch_thread_rwlock_unlock(member->rwlock);
                ...
                stream->write_function(stream, "-ERR  Invalid Member ID : %d", i);
        }
